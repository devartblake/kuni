    ...
    if (this.isSupportedPlatform) {
      try {
        autoUpdater.on('update-downloaded', (evt) => {
          windowManager.dojoWindow.squirrelUpdateDownloaded()
        })
        autoUpdater.on('error', (evt) => {
          if (this.isSigningFailureException(evt)) {
            console.log('Autoupdater disabled:', evt.message)
            windowManager.dojoWindow.squirrelUpdateDisabled()
          } else {
            windowManager.dojoWindow.squirrelUpdateError()
          }
        })
        autoUpdater.on('update-available', (evt) => {
          windowManager.dojoWindow.squirrelUpdateAvailable()
        })
        autoUpdater.on('update-not-available', (evt) => {
          windowManager.dojoWindow.squirrelUpdateNotAvailable()
        })
        autoUpdater.on('checking-for-update', (evt) => {
          windowManager.dojoWindow.squirrelCheckingForUpdate()
        })
