...
class StorageContainer extends EventEmitter {
  ...
  constructor (containerName) {
    super()
    this.__path__ = path.join(DB_DIR_PATH, containerName + '_db.json')
    this.__writeHold__ = null
    this.__writeLock__ = false
    this.__data__ = undefined
    this.__lastBackup__ = 0
    this.__ipcReplyChannel__ = `storageContainer:${containerName}:reply`

    this._loadFromDiskSync()

    ipcMain.on(`storageContainer:${containerName}:setItem`, this._handleIPCSetItem.bind(this))
    ipcMain.on(`storageContainer:${containerName}:removeItem`, this._handleIPCRemoveItem.bind(this))
    ipcMain.on(`storageContainer:${containerName}:getItem`, this._handleIPCGetItem.bind(this))
    ipcMain.on(`storageContainer:${containerName}:allKeys`, this._handleIPCAllKeys.bind(this))
    ipcMain.on(`storageContainer:${containerName}:allItems`, this._handleIPCAllItems.bind(this))
    ipcMain.on(`storageContainer:${containerName}:getStats`, this._handleIPCGetStats.bind(this))
    ipcMain.on(`storageContainer:${containerName}:measurePerformance`, this._handleIPCMeasurePerformance.bind(this))
  }
