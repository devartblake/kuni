...
var chatApiPool = 1000; // This is the default rate limit set by mixer and will update.
var chatApiReset = 0;   // This is the time the rate limit will redresh. Will be updated.
var groupWaitList = [];
...
/** Clear User Queue **/
// This will be used by disconnect function to clear the wait list.
function clearGroupQueue() {
    var groupWaitList = [];
}

/** Chat API Checker **/
// This checks every x seconds to see if we have any API calls left, and if so it will send a username on to br grouped.
...
function autoGroup(username, userid, participant){
    ...
    var customGroups = []; // We set banned here because it's always true.
    ...
    try{
        ...
        for (scene in gameScenes){
            ...
            // Also make "banned" a custom group.
            customGroups.push('banned');
        ...
        if(isUserGrouped === false){
            Chat.getUser(userid, function(response){
                ...
                } else if (mainGroup == "Mod" && modGroup === true || mainGroup == "Owner" && modGroup === true){
